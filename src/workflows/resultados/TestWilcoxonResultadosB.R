...
for (i in 1:length(filesb)) {
    # Evaluar el test de Wilcoxon para todas las filas
      for (k in 1:length(get(filesb[i]))) {
          for (l in 1:length(get(filesb[i]))) {
              # Verifico que las filas no contengan NA
              if (any(is.na(get(filesb[i])[k,])) || any(is.na(get(filesb[i])[l,]))) {
                  next
              }
              result <- wilcoxon_test(get(filesb[i])[k,], get(filesb[i])[l,])
              ...
              promedio1 <- mean(as.numeric(unlist(get(filesb[i])[k, -1])))
              promedio2 <- mean(as.numeric(unlist(get(filesb[i])[l, -1])))
              # Guardar el resultado en el data frame
              resultados <- rbind(resultados, data.frame(exp1 = paste0(filesb[i],"-",get(filesb[i])[k,]$envios), exp2 = paste0(filesb[i],"-",get(filesb[i])[l,]$envios), p.value = result$p.value, promedio1 = promedio1, promedio2 = promedio2))
          }
  }
}
...
# elimino las filas donde exp1 y exp2 son iguales
resultados <- resultados[resultados$exp1 != resultados$exp2,]
...
write.csv(resultados2, "experimentos-b-pvalue.csv", row.names = TRUE)

# guardo resultados 3 en un archivo csv experimentos-mayor-promedio.csv
write.csv(resultados3, "experimentos-b-mayor-promedio.csv", row.names = TRUE)
...
write.csv(resultados4, "experimentos-b-cantidad.csv", row.names = TRUE)
