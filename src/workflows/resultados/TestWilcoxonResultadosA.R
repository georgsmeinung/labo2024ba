...
for (i in 1:length(filesa)) {
    # Evaluar el test de Wilcoxon para todas las filas
      for (k in 1:length(get(filesa[i]))) {
          for (l in 1:length(get(filesa[i]))) {
              # Verifico que las filas no contengan NA
              if (any(is.na(get(filesa[i])[k,])) || any(is.na(get(filesa[i])[l,]))) {
                  next
              }
              result <- wilcoxon_test(get(filesa[i])[k,], get(filesa[i])[l,])
              ...
              promedio2 <- mean(as.numeric(unlist(get(filesa[i])[l, -1])))
              # Guardar el resultado en el data frame
              resultados <- rbind(resultados, data.frame(exp1 = paste0(filesa[i],"-",get(filesa[i])[k,]$envios), exp2 = paste0(filesa[i],"-",get(filesa[i])[l,]$envios), p.value = result$p.value, promedio1 = promedio1, promedio2 = promedio2))
          }
  }
}
...
write.csv(resultados2, "experimentos-a-pvalue.csv", row.names = TRUE)

# guardo resultados 3 en un archivo csv experimentos-mayor-promedio.csv
write.csv(resultados3, "experimentos-a-mayor-promedio.csv", row.names = TRUE)
...
write.csv(resultados4, "experimentos-a-cantidad.csv", row.names = TRUE)
